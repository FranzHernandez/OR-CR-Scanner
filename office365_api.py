...
@st.cache_data
def base64decoder(encrypted_string:str):
    encrypted_string_bytes = encrypted_string.encode("ascii")

    base64_bytes = base64.b64decode(encrypted_string_bytes)
    base64_string = base64_bytes.decode("ascii")

    return base64_string

USERNAME = base64decoder(st.secrets['sharepoint_email'])
PASSWORD = base64decoder(st.secrets['sharepoint_password'])
SHAREPOINT_SITE = base64decoder(st.secrets['sharepoint_url_site'])
SHAREPOINT_SITE_NAME = base64decoder(st.secrets['sharepoint_site_name'])
SHAREPOINT_DOC = base64decoder(st.secrets['sharepoint_doc_library'])
